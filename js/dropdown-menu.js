...
function globalNavDropdowns(e) {
	...
	this.root = this.container.querySelector(".site-header > .wrap");
	if (!this.root) {
		return;
	}

	this.primaryNav = this.container.querySelector(".nav-primary .menu");
	if (!this.primaryNav) {
		return;
	}

	this.primaryNavItem = this.container.querySelector(
		".nav-primary .menu-item:first-child a"
	);
	if (!this.primaryNavItem) {
		return;
	}

	this.secondaryNavItem = this.container.querySelector(
		".nav-primary-cta .menu .menu-item:first-child a"
	);
	if (!this.secondaryNavItem) {
		return;
	}
...
function globalNavPopup(e) {
	...
	(this.activeClass = "activated"),
		(this.root = document.querySelector(e)),
		(this.link = this.root.querySelector(".menu-toggle")),
		(this.popup = this.root.querySelector(".popup")),
		/*this.closeButton = this.root.querySelector(".popupCloseButton"),*/ this.link.addEventListener(
			n,
			function (e) {
...
(Strut.isRetina = window.devicePixelRatio > 1.3),
	...
	(globalNavPopup.prototype.togglePopup = function () {
		...
		if (e) {
			...
			this.link.setAttribute("aria-pressed", false);
		} else {
			...
			this.link.setAttribute("aria-pressed", true);
		}
	}),
	...
	Strut.ready(function () {
		new globalNavDropdowns(".globalNav"),
			...
			); /*, new globalNavPopup(".globalFooterNav .select.country"), new globalNavPopup(".globalFooterNav .select.language")*/
	});
