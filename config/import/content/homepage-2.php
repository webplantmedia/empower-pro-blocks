<?php
/**
 * Empower Pro Blocks child theme.
 *
 * @package Empower_Pro_Blocks
 * @author  Web Plant Media
 * @license GPL-2.0+
 * @link    https://webplantmedia.com/product/empower-pro-blocks/
 */

$empower_pro_blocks_image_1  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-1.jpg';
$empower_pro_blocks_image_2  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-2.jpg';
$empower_pro_blocks_image_3  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-3.jpg';
$empower_pro_blocks_image_4  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-4.jpg';
$empower_pro_blocks_image_5  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-5.jpg';
$empower_pro_blocks_image_6  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-6.jpg';
$empower_pro_blocks_image_7  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-7.jpg';
$empower_pro_blocks_image_8  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-8.jpg';
$empower_pro_blocks_image_9  = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-9.jpg';
$empower_pro_blocks_image_10 = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-10.jpg';
$empower_pro_blocks_image_11 = EMPOWER_PRO_BLOCKS_URL . 'config/import/images/image-11.jpg';

$empower_pro_blocks_content = <<<CONTENT
<!-- wp:spacer {"height":25} -->
<div style="height:25px" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->

<!-- wp:shortcode -->
[soliloquy slug="home-page-slider"]
<!-- /wp:shortcode -->

<!-- wp:atomic-blocks/ab-columns {"columns":3,"layout":"ab-3-col-equal","columnsGap":1,"align":"wide","marginTop":15} -->
<div class="wp-block-atomic-blocks-ab-columns ab-layout-columns-3 ab-3-col-equal alignwide" style="margin-top:15px"><div class="ab-layout-column-wrap ab-block-layout-column-gap-1 ab-is-responsive-column"><!-- wp:atomic-blocks/ab-column -->
<div class="wp-block-atomic-blocks-ab-column ab-block-layout-column"><div class="ab-block-layout-column-inner"><!-- wp:atomic-blocks/ab-cta {"buttonText":"Shop","buttonBackgroundColor":"#ffffff","buttonTextColor":"#464646","buttonShape":"ab-button-shape-square","imgID":132,"dimRatio":80,"className":"image-link"} -->
<div style="text-align:center" class="wp-block-atomic-blocks-ab-cta image-link ab-block-cta"><div class="ab-cta-image-wrap"><img class="ab-cta-image has-background-dim-80 has-background-dim" src="http://demo.webplantmedia.com/empower-pro-blocks/wp-content/uploads/sites/2/2019/01/pexels-photo-1170659.jpeg" alt=""/></div><div class="ab-cta-content"><h2 class="ab-cta-title ab-font-size-32" style="color:#32373c">Hello</h2><div class="ab-cta-text ab-font-size-32" style="color:#32373c"><p>fadsfsdf</p><p>fdsfdsdf</p></div></div><div class="ab-cta-button"><a href="#" target="_self" rel="noopener noreferrer" class="ab-button ab-button-shape-square ab-button-size-medium" style="color:#464646;background-color:#ffffff">Shop</a></div></div>
<!-- /wp:atomic-blocks/ab-cta --></div></div>
<!-- /wp:atomic-blocks/ab-column -->

<!-- wp:atomic-blocks/ab-column -->
<div class="wp-block-atomic-blocks-ab-column ab-block-layout-column"><div class="ab-block-layout-column-inner"><!-- wp:atomic-blocks/ab-cta {"buttonText":"About","buttonBackgroundColor":"#ffffff","buttonTextColor":"#464646","buttonShape":"ab-button-shape-square","imgID":129,"dimRatio":80,"className":"image-link"} -->
<div style="text-align:center" class="wp-block-atomic-blocks-ab-cta image-link ab-block-cta"><div class="ab-cta-image-wrap"><img class="ab-cta-image has-background-dim-80 has-background-dim" src="http://demo.webplantmedia.com/empower-pro-blocks/wp-content/uploads/sites/2/2019/01/pexels-photo-1207918.jpeg" alt=""/></div><div class="ab-cta-content"><h2 class="ab-cta-title ab-font-size-32" style="color:#32373c">Testing Heading</h2><div class="ab-cta-text ab-font-size-32" style="color:#32373c"><p></p></div></div><div class="ab-cta-button"><a href="#" target="_self" rel="noopener noreferrer" class="ab-button ab-button-shape-square ab-button-size-medium" style="color:#464646;background-color:#ffffff">About</a></div></div>
<!-- /wp:atomic-blocks/ab-cta -->

<!-- wp:paragraph -->
<p></p>
<!-- /wp:paragraph --></div></div>
<!-- /wp:atomic-blocks/ab-column -->

<!-- wp:atomic-blocks/ab-column -->
<div class="wp-block-atomic-blocks-ab-column ab-block-layout-column"><div class="ab-block-layout-column-inner"><!-- wp:atomic-blocks/ab-cta {"buttonText":"Contact","buttonBackgroundColor":"#ffffff","buttonTextColor":"#464646","buttonShape":"ab-button-shape-square","imgID":131,"dimRatio":80,"className":"image-link"} -->
<div style="text-align:center" class="wp-block-atomic-blocks-ab-cta image-link ab-block-cta"><div class="ab-cta-image-wrap"><img class="ab-cta-image has-background-dim-80 has-background-dim" src="http://demo.webplantmedia.com/empower-pro-blocks/wp-content/uploads/sites/2/2019/01/pexels-photo-312420.jpeg" alt=""/></div><div class="ab-cta-content"><h2 class="ab-cta-title ab-font-size-32" style="color:#32373c"></h2><div class="ab-cta-text ab-font-size-32" style="color:#32373c"><p></p></div></div><div class="ab-cta-button"><a href="#" target="_self" rel="noopener noreferrer" class="ab-button ab-button-shape-square ab-button-size-medium" style="color:#464646;background-color:#ffffff">Contact</a></div></div>
<!-- /wp:atomic-blocks/ab-cta --></div></div>
<!-- /wp:atomic-blocks/ab-column --></div></div>
<!-- /wp:atomic-blocks/ab-columns -->

<!-- wp:spacer {"height":50} -->
<div style="height:50px" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->

<!-- wp:columns {"align":"wide","className":"hero-text-button"} -->
<div class="wp-block-columns alignwide has-2-columns hero-text-button"><!-- wp:column -->
<div class="wp-block-column"><!-- wp:heading {"level":3,"className":"h4"} -->
<h3 class="h4">Premium Roasted Coffee</h3>
<!-- /wp:heading -->

<!-- wp:paragraph {"fontSize":"huge"} -->
<p class="has-huge-font-size">You'll love our selection of over 50 single origins, blends and flavors.</p>
<!-- /wp:paragraph --></div>
<!-- /wp:column -->

<!-- wp:column -->
<div class="wp-block-column"><!-- wp:spacer {"height":90} -->
<div style="height:90px" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->

<!-- wp:button {"backgroundColor":"secondary"} -->
<div class="wp-block-button"><a class="wp-block-button__link has-background has-secondary-background-color">Shop Coffee Grounds</a></div>
<!-- /wp:button --></div>
<!-- /wp:column --></div>
<!-- /wp:columns -->

<!-- wp:columns {"columns":3,"align":"wide"} -->
<div class="wp-block-columns alignwide has-3-columns"><!-- wp:column -->
<div class="wp-block-column"><!-- wp:html -->
<i class="ion-coffee"></i>
<!-- /wp:html -->

<!-- wp:heading {"level":4} -->
<h4>Superior Quality</h4>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Our great-tasting gourmet coffee begins with superior quality beans from all over the world. We carefully sample each lot of beans to ensure that you only get the best of the best.</p>
<!-- /wp:paragraph --></div>
<!-- /wp:column -->

<!-- wp:column -->
<div class="wp-block-column"><!-- wp:html -->
<i class="ion-thermometer"></i>
<!-- /wp:html -->

<!-- wp:heading {"level":4} -->
<h4>Precision Roasting</h4>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>We roast in small batches so we can manually control all stages of the roasting process. We roast by hand and maintain strict quality control to ensure a consistent cup.</p>
<!-- /wp:paragraph --></div>
<!-- /wp:column -->

<!-- wp:column -->
<div class="wp-block-column"><!-- wp:html -->
<i class="ion-bag"></i>
<!-- /wp:html -->

<!-- wp:heading {"level":4} -->
<h4>Fast Delivery</h4>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>To ensure that our customers only experience the freshest possible coffee, we delivers coffee to grocery stores within 72 hours of roasting, and usually ships customer's orders to their homes within 24 hours.</p>
<!-- /wp:paragraph --></div>
<!-- /wp:column --></div>
<!-- /wp:columns -->

<!-- wp:quote {"className":"is-style-large"} -->
<blockquote class="wp-block-quote is-style-large"><p>This company is top notch. The coffee is quality and they ship  it to you immediately. And when you have any questions they respond  quickly. Their website is easy to use as well. </p><cite>- Jessica S.</cite></blockquote>
<!-- /wp:quote -->

<!-- wp:button {"backgroundColor":"secondary","align":"center","className":"hero-button is-style-default"} -->
<div class="wp-block-button aligncenter hero-button is-style-default"><a class="wp-block-button__link has-background has-secondary-background-color">Browse our Coffee Selection</a></div>
<!-- /wp:button -->

<!-- wp:image {"id":2235,"align":"full"} -->
<figure class="wp-block-image alignfull"><img src="$empower_pro_blocks_image_2" alt="" class="wp-image-2235"/></figure>
<!-- /wp:image -->

<!-- wp:spacer {"height":40} -->
<div style="height:40px" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->

<!-- wp:heading {"level":3,"className":"h4 alignwide"} -->
<h3 class="h4 alignwide">About Us</h3>
<!-- /wp:heading -->

<!-- wp:paragraph {"fontSize":"huge","className":"alignwide"} -->
<p class="has-huge-font-size alignwide">We focus our efforts on learning, implementing, and mastering coffee roasting and brewing techniques. We take the craft approach to everything we do and it shows in every cup.</p>
<!-- /wp:paragraph -->

<!-- wp:media-text {"mediaId":2254,"mediaType":"image","isStackedOnMobile":true} -->
<div class="wp-block-media-text alignwide is-stacked-on-mobile"><figure class="wp-block-media-text__media"><img src="$empower_pro_blocks_image_9" alt="" class="wp-image-2254"/></figure><div class="wp-block-media-text__content"><!-- wp:paragraph {"placeholder":"Content…"} -->
<p>We are a small team of artisans from across the country that came together to share a common passion for specialty coffee. We are dedicated to:</p>
<!-- /wp:paragraph -->

<!-- wp:list -->
<ul><li>Sourcing the finest quality beans</li><li>Carefully roasting to perfection</li><li>24 hour shipping and delivery</li><li>Fast and friendly customer support</li><li>Giving back to the community</li></ul>
<!-- /wp:list -->

<!-- wp:button {"backgroundColor":"secondary"} -->
<div class="wp-block-button"><a class="wp-block-button__link has-background has-secondary-background-color">More about our company →</a></div>
<!-- /wp:button --></div></div>
<!-- /wp:media-text -->

<!-- wp:quote {"className":"is-style-large"} -->
<blockquote class="wp-block-quote is-style-large"><p>I have been a customer since 2013. I love this company. They are quick with getting the coffee in the  mail and the product is fresh and ready to use. Very satisfied. </p><cite>- Adam V.</cite></blockquote>
<!-- /wp:quote -->

<!-- wp:button {"backgroundColor":"secondary","align":"center","className":"hero-button is-style-default"} -->
<div class="wp-block-button aligncenter hero-button is-style-default"><a class="wp-block-button__link has-background has-secondary-background-color">Browse our Coffee Selection</a></div>
<!-- /wp:button -->

<!-- wp:image {"id":2222,"align":"full"} -->
<figure class="wp-block-image alignfull"><img src="$empower_pro_blocks_image_3" alt="" class="wp-image-2222"/></figure>
<!-- /wp:image -->

<!-- wp:spacer {"height":40} -->
<div style="height:40px" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->

<!-- wp:heading {"level":3,"className":"h4 alignwide"} -->
<h3 class="h4 alignwide">Our Best Selling Coffees</h3>
<!-- /wp:heading -->

<!-- wp:paragraph {"fontSize":"huge","className":"alignwide"} -->
<p class="has-huge-font-size alignwide">Shipping to Your Door</p>
<!-- /wp:paragraph -->

<!-- wp:woocommerce/product-new {"rows":2,"className":"alignwide"} /-->

<!-- wp:quote {"className":"is-style-large"} -->
<blockquote class="wp-block-quote is-style-large"><p>I have been a regular customer several years now.  The shipment has always arrived expediently and in good condition! The  product itself is highest quality every time! </p><cite>- Mary M.</cite></blockquote>
<!-- /wp:quote -->

<!-- wp:button {"backgroundColor":"secondary","align":"center","className":"hero-button is-style-default"} -->
<div class="wp-block-button aligncenter hero-button is-style-default"><a class="wp-block-button__link has-background has-secondary-background-color">Browse our Coffee Selection</a></div>
<!-- /wp:button -->

<!-- wp:image {"id":2226,"align":"full"} -->
<figure class="wp-block-image alignfull"><img src="$empower_pro_blocks_image_4" alt="" class="wp-image-2226"/></figure>
<!-- /wp:image -->

<!-- wp:spacer {"height":40} -->
<div style="height:40px" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->

<!-- wp:heading {"level":3,"className":"h4 alignwide"} -->
<h3 class="h4 alignwide"><a href="#">Read More →</a></h3>
<!-- /wp:heading -->

<!-- wp:paragraph {"fontSize":"huge","className":"alignwide"} -->
<p class="has-huge-font-size alignwide">Latest From Our Blog </p>
<!-- /wp:paragraph -->

<!-- wp:atomic-blocks/ab-post-grid {"postsToShow":2,"align":"wide","readMoreText":"Continue Reading →"} /-->
CONTENT;

return $empower_pro_blocks_content;
